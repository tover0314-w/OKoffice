...
export type OKofficeFetch = (input: string | URL, init?: RequestInit) => Promise<Response>;

export interface OKofficeClientOptions {
  baseUrl?: string;
  fetch?: OKofficeFetch;
}

export class OKofficeHttpError extends Error {
  readonly status: number;
  readonly body: unknown;

  constructor(message: string, status: number, body: unknown) {
    super(message);
    this.name = "OKofficeHttpError";
    this.status = status;
    this.body = body;
  }
}
...
export class OKofficeClient {
  readonly baseUrl: string;
  private readonly fetchImpl: OKofficeFetch;

  constructor(options: OKofficeClientOptions = {}) {
    this.baseUrl = normalizeBaseUrl(
      options.baseUrl ?? process.env.OKOFFICE_BASE_URL ?? process.env.AGENTPDF_BASE_URL ?? DEFAULT_BASE_URL,
    );
    const fetchImpl = options.fetch ?? globalThis.fetch;
    if (typeof fetchImpl !== "function") {
      throw new Error("OKofficeClient requires a fetch implementation.");
    }
    this.fetchImpl = fetchImpl.bind(globalThis) as OKofficeFetch;
  }
...
async function parseJsonResponse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok && !isToolResult(body)) {
    throw new OKofficeHttpError(
      `OKoffice API request failed with HTTP ${response.status}`,
      response.status,
      body,
    );
  }
...
/** @deprecated Use OKofficeClient */
export type AgentPDFClient = OKofficeClient;
/** @deprecated Use OKofficeClientOptions */
export type AgentPDFClientOptions = OKofficeClientOptions;
/** @deprecated Use OKofficeFetch */
export type AgentPDFFetch = OKofficeFetch;
/** @deprecated Use OKofficeHttpError */
export type AgentPDFHttpError = OKofficeHttpError;
