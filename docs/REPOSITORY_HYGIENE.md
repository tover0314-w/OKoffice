...
- Small fixture PDFs under `tests/fixtures/` when they are generated or license-safe.
...
- Local output directories: `.agentpdf-out/`, `/outputs/`, `/artifacts/`, `/artifact-bundles/`, `renders/`.
...
- Temporary data: `*.log`, `*.tmp`, `*.sqlite`, `*.db`, archives, ad hoc generated PDFs, benchmark dumps.

## Generated PDFs

Generated PDFs should usually live under `.agentpdf-out/` and remain untracked.

Commit a generated PDF only when all of these are true:
...
`examples/generated/hello.pdf` is the baseline example for this exception.
