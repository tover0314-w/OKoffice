...
def scan_to_pdf(image_paths: list[str | Path], output_path: str | Path) -> ToolResult:
    ...
    if not image_paths:
        raise OKofficeException("unsafe_input_rejected", "At least one scan image is required.")
...
def ocr_pdf(
    ...
    try:
        pages_usage = []
        all_regions = []
        with tempfile.TemporaryDirectory(prefix="okoffice-ocr-") as temp_dir:
...
def remove_existing_ocr_pdf(input_path: str | Path, output_path: str | Path) -> ToolResult:
    ...
    for page in reader.pages:
        writer.add_page(page)
    writer.add_metadata({"/Producer": "OKoffice local OCR-layer rewrite"})
    output = resolve_output_path(output_path)
    with output.open("wb") as handle:
        writer.write(handle)
    artifact = build_artifact(output, source_tool=tool)
...
def multilingual_ocr_pdf(
    ...
    writer.add_metadata(
        {
            "/Producer": "OKoffice local multilingual OCR placeholder",
            "/OKofficeOCRLanguages": ",".join(requested_languages),
        }
    )
    output = resolve_output_path(output_path)
    with output.open("wb") as handle:
...
def _render_pdf_page_to_image(source: Path, page_index: int, output_path: Path, scale: float) -> None:
    try:
        ...
        raise OKofficeException(
            ...
            retry_hint="Install the default OKoffice dependencies and retry.",
        ) from exc
    ...
    try:
        ...
        raise OKofficeException("pdf_render_failed", f"Unable to render page {page_index + 1}.") from exc
    finally:
