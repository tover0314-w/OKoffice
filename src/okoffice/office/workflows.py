...
from okoffice.office.deck import export_deck_pptx, render_deck_html, validate_deck_html_preview, validate_deck_presentation
...
def docset_to_sheet(
    *,
    ...
    except (OKofficeException, json.JSONDecodeError) as exc:
        error = exc.to_error() if isinstance(exc, OKofficeException) else OKofficeError(code="invalid_input", message=str(exc))
    ...
    for file_index, file_path in enumerate(files, start=1):
        ...
        if inspect_result.status == "failed":
            return _failed(
                ...
                or OKofficeError(code="unsupported_file_type", message=f"Unable to inspect board pack file: {file_path}"),
                tool=BOARD_PACK_TOOL_NAME,
            )
...
def source_to_board_pack(
    *,
    ...
    step1 = docset_to_sheet(
        ...
        title=title or "OKoffice Source To Board Pack Evidence",
        intent=intent or "Extract evidence from sources for board pack generation.",
    )
    if step1.status != "succeeded":
        return _failed_from_workflow_step(SOURCE_TO_BOARD_PACK_TOOL_NAME, step1, [step1])

    step2 = sheet_to_deck(
        workbook_path=xlsx_path,
        output_path=pptx_path,
        title=deck_title or title or "Board Pack Deck",
        max_rows_per_sheet=max_rows_per_sheet,
    )
    if step2.status != "succeeded":
        return _failed_from_workflow_step(SOURCE_TO_BOARD_PACK_TOOL_NAME, step2, [step1, step2])

    step3 = board_pack(
        files=[xlsx_path, pptx_path],
        output_path=output,
        title=title,
    )
    if step3.status != "succeeded":
        return _failed_from_workflow_step(SOURCE_TO_BOARD_PACK_TOOL_NAME, step3, [step1, step2, step3])
    ...
    checks = [
        ...
        ValidationCheck(
            ...
            details={"job_id": step3.job_id, "zip_path": output.as_posix()},
        ),
        ValidationCheck(
    ...
    return ToolResult(
        job_id=_job_id(),
        status="succeeded",
        tool=SOURCE_TO_BOARD_PACK_TOOL_NAME,
...
def _workflow_schema_object(schema: dict[str, Any] | str | Path) -> dict[str, Any]:
    ...
    if isinstance(schema, Path) or (isinstance(schema, str) and Path(schema).exists()):
        ...
        if not isinstance(data, dict):
            raise OKofficeException("invalid_input", "Schema JSON must be an object.")
        return data
    if isinstance(schema, str):
        ...
        if not isinstance(data, dict):
            raise OKofficeException("invalid_input", "Schema JSON must be an object.")
        return data
    raise OKofficeException("invalid_input", "schema must be a JSON object, JSON string, or path.")
...
def _failed_from_workflow_step(tool: str, failed_result: ToolResult, step_results: list[ToolResult]) -> ToolResult:
    error = failed_result.error or OKofficeError(
    ...
    *,
    ...
    if isinstance(payload, dict):
        ...
        try:
            ...
            raise OKofficeException("invalid_context_packet", f"Context packet JSON is invalid: {path}") from exc
    if not isinstance(packet, dict) or not isinstance(packet.get("items"), list):
        raise OKofficeException("invalid_context_packet", "Context packet must include an items array.")
    if not isinstance(packet.get("source_graph"), dict):
        raise OKofficeException("invalid_context_packet", "Context packet must include a source_graph object.")
    return packet
...
def _failed(error: OKofficeError, *, tool: str = TOOL_NAME) -> ToolResult:
    return ToolResult(
        job_id=_job_id(),
        status="failed",
        tool=tool,
