...
def _load_context_packet(context_packet_or_path: dict[str, Any] | str | Path) -> dict[str, Any]:
    ...
    if not isinstance(packet, dict) or not packet.get("context_packet_id"):
        raise OKofficeException("invalid_context_packet", "Context packet must include context_packet_id.")
...
def _schema_fields(schema: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(raw_fields, list) or not raw_fields:
        raise OKofficeException("invalid_input", "schema.fields must include at least one field.")
    fields: list[dict[str, Any]] = []
    for raw_field in raw_fields:
        if not isinstance(raw_field, dict) or not raw_field.get("name"):
            raise OKofficeException("invalid_input", "Every schema field must be an object with a name.")
        fields.append(
            {
