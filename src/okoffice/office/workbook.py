    ...
    *,
    ...
    if evidence is None:
        if evidence_path is None:
            raise OKofficeException("invalid_input", "evidence_path or evidence is required.")
    ...
    if not isinstance(raw, dict):
        raise OKofficeException("invalid_input", "Evidence JSON must be an object.")
    candidate = raw.get("usage", {}).get("extraction") if isinstance(raw.get("usage"), dict) else raw
    if not isinstance(candidate, dict):
        raise OKofficeException("invalid_input", "Evidence JSON must include an extraction object.")
    extraction = EvidenceExtraction.model_validate(candidate)
    if not extraction.fields:
        raise OKofficeException("invalid_input", "Evidence extraction must include at least one field.")
