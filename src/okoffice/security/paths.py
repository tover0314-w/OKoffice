...
def resolve_input_path(path: str | Path) -> Path:
    ...
    if not resolved.exists() or not resolved.is_file():
        raise OKofficeException("file_not_found", f"Input file not found: {candidate}")
    return resolved
...
def _reject_suspicious_path(path: Path) -> None:
    if any(part == ".." for part in path.parts):
        raise OKofficeException("unsafe_input_rejected", f"Path traversal is not allowed: {path}")
    as_text = str(path)
    if any(char in as_text for char in SUSPICIOUS_CHARS):
        raise OKofficeException("unsafe_input_rejected", f"Suspicious path rejected: {path}")
