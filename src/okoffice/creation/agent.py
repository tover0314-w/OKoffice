...
from okoffice.compose.context import DEFAULT_TARGET_PROFILES
from okoffice.context.classify import classify_context
from okoffice.core.pdf import BUILTIN_STYLE_PACKS, SUPPORTED_IMAGE_SUFFIXES, create_markdown_pdf
...
from okoffice.renderers.html_package import render_html_package, write_composition_html_package
                    ...
                    "sections": [
                        {
                            ...
                            "body": "OKoffice turns heterogeneous context into validated PDF artifacts.",
                        },
                        {
                            "heading": "Evidence",
...
def create_pdf_from_prompt(
    ...
    if not prompt.strip():
        raise OKofficeException("invalid_input", "prompt is required for PDF creation.")
...
def plan_template_pack_creation(
    ...
    if not candidates:
        raise OKofficeException("invalid_template_pack", "Template pack contains no template candidates.")
    ...
    planned_pdf = str(planned_output_path) if planned_output_path is not None else (
        f".okoffice-out/{selected['template_id']}.pdf"
    )
    create_payload = {
...
def create_pdf_with_agent(
    ...
    context_packet: dict[str, Any] | str | Path | None = None,
    context_packet_path: str | Path | None = None,
    output_path: str | Path = ".okoffice-out/create-agent.pdf",
...
def _load_context_packet_for_template(
    ...
    if context_packet is not None and context_packet_path is not None:
        raise OKofficeException("invalid_input", "Use context_packet or context_packet_path, not both.")
    ...
    if not isinstance(raw_packet, dict):
        raise OKofficeException("invalid_context_packet", "Context packet must be a JSON object.")
...
def _agent_blocks_from_raw(
    ...
    if not isinstance(raw_blocks, list):
        raise OKofficeException("invalid_input", "data.blocks must be an array when provided.")

    seen = set(seen_block_ids or set())
    blocks: list[dict[str, Any]] = []
    for index, raw_block in enumerate(raw_blocks, start=1):
        if not isinstance(raw_block, dict):
            raise OKofficeException("invalid_input", f"data.blocks[{index - 1}] must be an object.")
...
def _agent_block_data(block_type: str, raw_block: dict[str, Any]) -> dict[str, Any]:
    if block_type == "code":
        ...
        if not code:
            raise OKofficeException("invalid_input", "code blocks require a non-empty code field.")
        return {
    ...
    if block_type == "table":
        ...
        if not columns:
            raise OKofficeException("invalid_input", "table blocks require columns or rows.")
        return {"columns": columns, "rows": rows}
    if block_type == "image":
        path = _stringify(raw_block.get("path") or raw_block.get("image_path"))
        if not path:
            raise OKofficeException("invalid_input", "image blocks require path or image_path.")
...
def _image_block_evidence(path: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise OKofficeException("dependency_missing", "Image block evidence requires Pillow.") from exc
...
def _load_template_pack(template_pack: dict[str, Any] | str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise OKofficeException("invalid_template_pack", "Template pack JSON must be an object.")
    return payload
...
def _template_pack_colors(
    ...
    if not isinstance(raw_colors, dict):
        raise OKofficeException("invalid_color_value", f"Color scheme must be an object: {selected}")
...
def render_template_markdown(
    ...
    if template_id not in CREATE_TEMPLATES:
        raise OKofficeException("invalid_template", f"Unknown create template: {template_id}")
    content = dict(data or {})
...
def _style_pack_with_colors(style_pack: str, colors: dict[str, str]) -> dict[str, Any]:
    if style_pack in BUILTIN_STYLE_PACKS:
        ...
        if not isinstance(raw, dict):
            raise OKofficeException("invalid_style_pack", "Style pack JSON must be an object.")
        pack = raw
...
def _normalize_colors(colors: dict[str, str] | None) -> dict[str, str]:
    ...
    for key, raw_value in colors.items():
        ...
        if len(hex_value) != 6 or any(char not in "0123456789abcdefABCDEF" for char in hex_value):
            raise OKofficeException("invalid_color_value", f"Invalid hex color: {raw_value}")
