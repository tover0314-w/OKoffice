...
def cite_claims(
    ...
    if not normalized_claims:
        raise OKofficeException("invalid_input", "claims must include at least one claim object.")
...
def _load_source_map_entries(value: dict[str, Any] | list[dict[str, Any]] | str | Path) -> list[dict[str, Any]]:
    ...
    if isinstance(value, list):
        ...
        path = Path(value)
        if not path.exists():
            raise OKofficeException("file_not_found", f"Source map JSON not found: {path}")
    ...
    if not isinstance(payload, list) or not all(isinstance(item, dict) for item in payload):
        raise OKofficeException("invalid_input", "source_map must be a JSON array or object containing source_map.")
    return payload
...
def _normalize_claims(claims: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, raw_claim in enumerate(claims, start=1):
        if not isinstance(raw_claim, dict):
            raise OKofficeException("invalid_input", "claims entries must be JSON objects.")
        ...
        if not text:
            raise OKofficeException("invalid_input", "Each claim must include text, claim, or quote.")
