...
from okoffice.authoring.html_deck import write_authoring_html_package, write_raw_html_package
from okoffice.authoring.pages import write_pages_from_storyboard
from okoffice.authoring.qa import visual_report
...
from okoffice.authoring.workflow import plan_research_deck_workflow
...
from okoffice.compose.context import (
...
from okoffice.compare.local import semantic_diff_pdf, version_report_pdf, visual_diff_pdf
...
from okoffice.context.web import capture_web_context
...
from okoffice.office.deck_writer import create_deck_presentation as create_deck_presentation_from_workbook
...
from okoffice.validation.pdf import (
...
from okoffice.workflows.createpdf import createpdf_html_first
...
def run_office_workflow_plan(
    goal: str,
    input_paths: list[str | Path] | None = None,
    output_paths: list[str | Path] | None = None,
) -> ToolResult:
    return plan_office_workflow(
        goal=goal,
        input_paths=[Path(path) for path in (input_paths or [])],
        output_paths=[Path(path) for path in (output_paths or [])],
    )
...
def run_word_create_report(
    ...
    if workbook_path is None:
        return ToolResult(
            ...
            error=OKofficeError(code="invalid_input", message="workbook_path is required."),
    ...
    if output_path is None:
        return ToolResult(
            ...
            status="failed",
            tool="word.create.report",
            error=OKofficeError(code="invalid_input", message="output_path is required."),
...
def run_sheet_write_workbook(
    ...
    *,
    ...
    if evidence_path is not None or evidence is not None:
        from okoffice.office.workbook import write_sheet_workbook as write_evidence_sheet_workbook
...
def run_deck_create_presentation(
    ...
    if workbook_path is None and outline_or_plan is None:
        return ToolResult(
            ...
            status="failed",
            tool="deck.create.presentation",
            error=OKofficeError(code="invalid_input", message="workbook_path or outline_or_plan is required."),
...
def run_deck_render_html(
    ...
    if payload is None:
        return ToolResult(
            ...
            status="failed",
            tool="deck.render.html",
            error=OKofficeError(code="invalid_input", message="plan_path, plan, or outline is required."),
