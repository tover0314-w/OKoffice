...
def create_app() -> FastAPI:
    ...
    def healthz() -> dict[str, str]:
        return {"status": "ok", "service": "okoffice"}
    ...
    def show_job(job_id: str) -> JSONResponse:
        ...
        if result is None:
            return _error_response(
                OKofficeError(code="file_not_found", message=f"Job not found: {job_id}"),
                status_code=404,
            )
        return JSONResponse(result.model_dump(mode="json"))
    ...
    def show_artifact(artifact_id: str) -> JSONResponse:
        ...
        if artifact is None:
            return _error_response(
                OKofficeError(code="file_not_found", message=f"Artifact not found: {artifact_id}"),
                status_code=404,
            )
    ...
    def download_artifact(artifact_id: str):
        ...
        if not artifact.path.exists():
            return _error_response(
                OKofficeError(code="file_not_found", message=f"Artifact file missing: {artifact.path}"),
                status_code=404,
            )
...
def _run_tool(tool_name: str, payload: dict[str, Any]) -> ToolResult:
    if tool_name == "agent.setup.claude_code":
        ...
        return run_agent_setup_claude_code(
            ...
            command=str(payload.get("command", "okoffice")),
            args_prefix=[str(arg) for arg in args_prefix] if isinstance(args_prefix, list) else None,
            server_name=str(payload.get("server_name", "okoffice")),
    ...
    if tool_name == "office.workflow.plan":
        input_paths = payload.get("input_paths", payload.get("inputs", payload.get("paths", [])))
        output_paths = payload.get("output_paths", payload.get("outputs", []))
    ...
    if tool_name == "office.workflow.docset_to_sheet":
        ...
        return run_office_workflow_docset_to_sheet(
            files=files if isinstance(files, list) else [],
            schema=payload.get("schema") or payload.get("schema_path") or {},
            output_path=payload.get("output_path", payload.get("output", ".okoffice-out/evidence.xlsx")),
            title=str(payload["title"]) if payload.get("title") is not None else None,
            intent=str(payload["intent"]) if payload.get("intent") is not None else None,
    ...
    if tool_name == "office.bundle.export":
        ...
        return run_office_bundle_export(
            ...
            output_path=payload.get("output_path", payload.get("output", ".okoffice-out/bundle.okoffice.zip")),
            title=str(payload["title"]) if payload.get("title") is not None else None,
    ...
    if tool_name == "sheet.profile.data":
    ...
    if tool_name == "pdf.workflow.createpdf":
        ...
        return run_workflow_createpdf(
            ...
            pages=str(payload.get("pages", "all")),
            renderer_backend=str(payload.get("renderer_backend", "auto")),
        )
    ...
    if tool_name == "pdf.context.web_capture":
        return run_context_web_capture(
            ...
            max_bytes=_optional_int(payload.get("max_bytes")) or 1_000_000,
    ...
    if tool_name == "pdf.patch.plan":
        ...
        return run_patch_plan(
            ...
            artifact_graph_path=payload.get("artifact_graph_path"),
...
def _coerce_optional_int(value: object, field_name: str) -> tuple[int | None, OKofficeError | None]:
    if value is None:
        return None, None
    try:
        return int(value), None
    except (TypeError, ValueError):
        return None, OKofficeError(
...
def _error_response(error: OKofficeError, status_code: int) -> JSONResponse:
