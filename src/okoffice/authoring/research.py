...
from okoffice.authoring.models import AuthoringBrief, EvidenceCard, SourceCard
...
def _require_source_list(value: Any) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise OKofficeException("unsafe_input_rejected", "Sources must be provided as a JSON array.")
    return value
...
def _safe_source_url(value: str | None) -> str | None:
    ...
    if parsed.scheme.lower() not in {"http", "https"}:
        raise OKofficeException("unsafe_input_rejected", "Source URLs must use http:// or https://.")
    if parsed.username or parsed.password:
        raise OKofficeException("unsafe_input_rejected", "Source URLs must not include credentials.")
    host = parsed.hostname
    if not host:
        raise OKofficeException("unsafe_input_rejected", "Source URLs must include a host.")
    try:
        port = parsed.port
    except ValueError as exc:
        raise OKofficeException("unsafe_input_rejected", "Source URL has an invalid port.") from exc
    netloc = host.lower()
...
def _failed_from_exception(tool: str, exc: OKofficeException) -> ToolResult:
    return ToolResult(
        job_id=_job_id(),
        status="failed",
        tool=tool,
