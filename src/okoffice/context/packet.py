...
def ingest_context_item(
    ...
    if not isinstance(context_item, dict) or not context_item:
        raise OKofficeException("invalid_input", "context_item must be a non-empty JSON object.")
...
def create_code_snapshot(
    ...
    if not source_path.exists():
        raise OKofficeException("file_not_found", f"Code snapshot path not found: {source_path}")
    if source_path.suffix.lower() not in CODE_EXTENSIONS:
        raise OKofficeException("invalid_context_item", f"Code snapshot path is not a code file: {source_path}")
...
def profile_data_source(
    ...
    source_path = Path(path).resolve()
    if not source_path.exists():
        raise OKofficeException("file_not_found", f"Data profile path not found: {source_path}")
    if source_path.suffix.lower() not in DATA_EXTENSIONS:
        raise OKofficeException("invalid_context_item", f"Data profile path is not a data file: {source_path}")
...
def _build_context_packet(
    ...
    if not context_items:
        raise OKofficeException("invalid_input", "context_items must include at least one item.")
...
def _normalize_context_item(raw: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    if raw.get("path") is None:
        raise OKofficeException("invalid_context_item", "Context item must include text, table, uri, url, or path.")

    path = Path(str(raw["path"])).resolve()
    if not path.exists():
        raise OKofficeException("file_not_found", f"Context item path not found: {path}")
...
def _code_snapshot_context_item(
    ...
    if start < 1 or end < start:
        raise OKofficeException("invalid_context_item", "line_start and line_end must define a valid line range.")
    if start > len(lines):
        raise OKofficeException("invalid_context_item", "line_start is beyond the end of the source file.")
...
def _json_table_preview(text: str, max_rows: int) -> dict[str, Any] | None:
    try:
        ...
        raise OKofficeException("invalid_context_item", "JSON data source is not parseable.") from exc
...
def _xlsx_table_preview(path: Path, sheet: str | None, max_rows: int) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    try:
        ...
        raise OKofficeException("invalid_context_item", f"XLSX workbook is missing a required part: {path}") from exc
    except BadZipFile as exc:
        raise OKofficeException("invalid_context_item", f"XLSX workbook is not a readable ZIP: {path}") from exc
    ...
    if requested:
        for sheet in sheets:
            if sheet["name"] == requested:
                return sheet
        raise OKofficeException("invalid_context_item", f"XLSX sheet not found: {requested}")
    return sheets[0]
...
def _normalize_inline_table(raw_table: Any) -> dict[str, Any]:
    if not isinstance(raw_table, dict):
        raise OKofficeException("invalid_context_item", "table context must be a JSON object.")
    raw_rows = raw_table.get("rows")
    if not isinstance(raw_rows, list):
        raise OKofficeException("invalid_context_item", "table context must include rows.")
    ...
    for raw_row in raw_rows:
        if isinstance(raw_row, dict):
            ...
            raise OKofficeException("invalid_context_item", "table rows must be arrays or objects.")
...
def _read_docx_text(path: Path) -> str:
    try:
        ...
        raise OKofficeException("invalid_context_item", f"DOCX document is not a readable ZIP: {path}") from exc

    try:
        root = ET.fromstring(document_xml)
    except ET.ParseError as exc:
        raise OKofficeException("invalid_context_item", f"DOCX document XML is not parseable: {path}") from exc
...
def _web_citation_evidence(uri: str, raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if not original_url:
        raise OKofficeException("invalid_context_item", "Web context link cannot be empty.")
    ...
    if scheme not in {"http", "https"}:
        raise OKofficeException("unsafe_input_rejected", "Web context links must use http:// or https:// URLs.")
    if parsed.username or parsed.password:
        raise OKofficeException("unsafe_input_rejected", "Web context links must not include credentials.")

    domain = (parsed.hostname or "").lower()
    if not domain:
        raise OKofficeException("unsafe_input_rejected", "Web context link must include a host.")
    try:
        port = parsed.port
    except ValueError as exc:
        raise OKofficeException("unsafe_input_rejected", "Web context link has an invalid port.") from exc
