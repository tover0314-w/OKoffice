...
from okoffice.schemas.models import ToolResult, ValidationReport
...
def compose_from_context(
    ...
    if renderer_mode == "html":
        ...
        rendered = create_slide_deck_pdf(
            slides,
            output_path=output_path,
            title=profile.get("title") or profile.get("name") or "OKoffice Slide Deck",
        ...
        rendered = create_markdown_pdf(
            markdown,
            output_path=output_path,
            title=profile.get("title") or profile.get("name") or "OKoffice Composition",
...
def render_composition_ir(
    ...
    if not isinstance(composition_ir, dict):
        raise OKofficeException("invalid_input", "composition_ir must be present to render composition IR.")
    ...
    effective_title = title or str(render_plan.get("title") or target_profile.get("title") or target_profile.get("name") or "OKoffice Composition")
...
def _render_plan(profile: dict[str, Any], markdown: str, slides: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    plan = {
        ...
        "title": str(profile.get("title") or profile.get("name") or "OKoffice Composition"),
        "style_pack": str(profile.get("style_pack") or "paper_ink"),
...
def _load_composition_payload(composition: dict[str, Any] | str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise OKofficeException("invalid_input", "Composition payload must be a JSON object.")
    return payload
...
def _resolve_target_profile(target_profile: dict[str, Any] | str) -> dict[str, Any]:
    ...
    if profile_id not in DEFAULT_TARGET_PROFILES:
        ...
        raise OKofficeException("invalid_target_profile", f"Unknown target profile: {profile_id}. Available: {available}.")
...
def _compose_slides(
    ...
    title = str(profile.get("title") or profile.get("name") or "OKoffice Slide Deck")
...
def _compose_markdown(
    ...
    title = str(profile.get("title") or profile.get("name") or packet.get("title") or "OKoffice Composition")
    items = list(packet.get("items", []))
    blocks: list[dict[str, Any]] = []
    source_map: list[dict[str, Any]] = []
    lines = [
