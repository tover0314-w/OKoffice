...
def url_to_pdf(
    ...
    *,
    ...
    if parsed.scheme == "file":
        ...
        request = Request(url, headers={"User-Agent": "OKoffice-Local/0.1"})
...
def pdf_to_html(
    ...
    document = "\n".join(
        [
            "<!doctype html>",
            '<html lang="en">',
            "<head><meta charset=\"utf-8\"><title>OKoffice HTML Export</title></head>",
            "<body>",
            *page_blocks,
            "</body></html>",
        ]
...
def _read_docx_paragraphs(path: Path) -> list[str]:
    if path.suffix.lower() != ".docx":
        raise OKofficeException("unsupported_file_type", "Only .docx files are supported.")
    try:
        with zipfile.ZipFile(path) as archive:
            xml = archive.read("word/document.xml")
    except Exception as exc:
        raise OKofficeException("pdf_parse_failed", f"Unable to parse DOCX: {path}") from exc
...
def _read_pptx_slides(path: Path) -> list[list[str]]:
    if path.suffix.lower() != ".pptx":
        raise OKofficeException("unsupported_file_type", "Only .pptx files are supported.")
    try:
        with zipfile.ZipFile(path) as archive:
            names = sorted(
        ...
        raise OKofficeException("pdf_parse_failed", f"Unable to parse PPTX: {path}") from exc


def _read_xlsx_rows(path: Path) -> list[list[str]]:
    if path.suffix.lower() != ".xlsx":
        raise OKofficeException("unsupported_file_type", "Only .xlsx files are supported.")
    try:
        with zipfile.ZipFile(path) as archive:
        ...
        raise OKofficeException("pdf_parse_failed", f"Unable to parse XLSX: {path}") from exc
