...
def setup_openai_agents(
    ...
    tool = "agent.setup.openai_agents"
    config = build_openai_agents_mcp_config(
        safe_root=safe_root,
        command=command,
    ...
    usage: dict[str, Any] = {
        ...
        "safe_root": safe_root,
        "mcp_config": config,
        "recommended_config_files": ["mcp.json", ".mcp.json"],
...
def build_openai_agents_mcp_config(
    safe_root: str = DEFAULT_SAFE_ROOT,
    command: str = "okoffice",
    args_prefix: list[str] | None = None,
    server_name: str = "okoffice",
    ...
    if not command:
        raise OKofficeException("unsafe_input_rejected", "OpenAI Agents MCP command cannot be empty.")
    if not server_name:
        raise OKofficeException("unsafe_input_rejected", "OpenAI Agents MCP server name cannot be empty.")
