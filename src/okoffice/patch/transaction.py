...
from okoffice.core.pdf import create_markdown_pdf, create_slide_deck_pdf
...
def plan_patch_transaction(
    ...
    if not source.exists():
        raise OKofficeException("file_not_found", f"Patch input PDF not found: {source}")
    if not operations:
...
def _html_layer_patch_context(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if first_layer is None:
        raise OKofficeException("invalid_patch", "HTML layer rerender requires html_layer_evidence.")
    ...
    if not manifest_path.exists():
        raise OKofficeException("file_not_found", f"HTML package manifest not found: {manifest_path}")
    if not html_path.exists():
        raise OKofficeException("file_not_found", f"HTML source not found: {html_path}")
    for operation in manifest["operations"]:
    ...
    if not isinstance(payload, dict):
        raise OKofficeException("html_invalid_package", "HTML package manifest must be a JSON object.")
...
def verify_patch_transaction(
    ...
    if not patched.exists():
        raise OKofficeException("file_not_found", f"Patched PDF not found: {patched}")
...
def _normalize_operation(operation: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    if op not in SUPPORTED_PATCH_OPERATIONS:
        raise OKofficeException("unsupported_patch_operation", f"Unsupported patch operation: {op}")
    ...
    if op == "append_markdown":
        ...
        if not markdown:
            raise OKofficeException("invalid_patch", "append_markdown operation requires markdown.")
        normalized["markdown"] = markdown
    elif op == "append_code_block":
        code = str(operation.get("code") or "").rstrip()
        if not code:
            raise OKofficeException("invalid_patch", "append_code_block operation requires code.")
        ...
        if not columns:
            raise OKofficeException("invalid_patch", "append_table operation requires columns.")
        if not isinstance(rows, list) or not rows:
            raise OKofficeException("invalid_patch", "append_table operation requires one or more rows.")
        ...
        if not raw_path:
            raise OKofficeException("invalid_patch", "append_image operation requires path.")
        image_path = Path(raw_path).resolve()
        if not image_path.exists():
            raise OKofficeException("file_not_found", f"Patch image not found: {image_path}")
        ...
        if raw_image_path:
            ...
            if not image_path.exists():
                raise OKofficeException("file_not_found", f"Patch slide image not found: {image_path}")
            normalized["image_path"] = image_path.as_posix()
    elif op == "append_citation":
        source = str(operation.get("source") or "").strip()
        if not source:
            raise OKofficeException("invalid_patch", "append_citation operation requires source.")
...
def _load_manifest(patch_manifest: dict[str, Any] | str | Path) -> dict[str, Any]:
    if isinstance(patch_manifest, dict):
        ...
        if not path.exists():
            raise OKofficeException("file_not_found", f"Patch manifest not found: {path}")
        manifest = json.loads(path.read_text(encoding="utf-8-sig"))
    if "patch_id" not in manifest or "operations" not in manifest:
        raise OKofficeException("invalid_patch", "Patch manifest must include patch_id and operations.")
...
def _validate_loaded_patch_manifest(manifest: dict[str, Any]) -> None:
    operations = manifest.get("operations")
    if not isinstance(operations, list) or not operations:
        raise OKofficeException("invalid_patch", "Patch manifest operations must be a non-empty array.")
    for operation in operations:
        if not isinstance(operation, dict):
            raise OKofficeException("invalid_patch", "Patch manifest operations must be objects.")
        op = str(operation.get("op") or "")
        if op not in SUPPORTED_PATCH_OPERATIONS:
            raise OKofficeException("unsupported_patch_operation", f"Unsupported patch operation: {op}")
    layer_policy_maps = []
...
def _validate_operation_layer_refs(
    ...
    if not isinstance(layers, list):
        raise OKofficeException("invalid_layer_manifest", "Template layer manifest layers must be an array.")
...
def _validate_operation_html_layer_refs(
    ...
    if not isinstance(html_layer_index, dict):
        raise OKofficeException("invalid_artifact_graph", "Artifact graph html_layer_index must be an object.")
...
def _load_composition_payload(composition_path: str | Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise OKofficeException("file_not_found", f"Composition artifact not found: {path}")
    payload = json.loads(path.read_text(encoding="utf-8-sig"))
    if not isinstance(payload, dict) or "composition_ir" not in payload:
        raise OKofficeException("invalid_composition_ir", "Composition payload must include composition_ir.")
    return payload
...
def _load_layer_manifest(layer_manifest_path: str | Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise OKofficeException("file_not_found", f"Template layer manifest not found: {path}")
    payload = json.loads(path.read_text(encoding="utf-8-sig"))
    if not isinstance(payload, dict) or "template_layer_manifest_id" not in payload:
        raise OKofficeException("invalid_layer_manifest", "Layer manifest must include template_layer_manifest_id.")
    return payload
...
def _load_artifact_graph(artifact_graph_path: str | Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise OKofficeException("file_not_found", f"Artifact graph not found: {path}")
    payload = json.loads(path.read_text(encoding="utf-8-sig"))
    if not isinstance(payload, dict) or "artifact_graph_version" not in payload:
        raise OKofficeException("invalid_artifact_graph", "Artifact graph must include artifact_graph_version.")
    return payload


def _source_map_by_ref(source_map: Any) -> dict[str, list[dict[str, Any]]]:
    if not isinstance(source_map, list):
        raise OKofficeException("invalid_composition_ir", "Composition source_map must be an array.")
...
def _normalize_table(table: Any) -> dict[str, list[Any]]:
    if not isinstance(table, dict):
        raise OKofficeException("invalid_patch", "append_slide table must be an object with columns and rows.")
    columns = _string_list(table.get("columns"))
    rows = table.get("rows")
    if not columns or not isinstance(rows, list):
        raise OKofficeException("invalid_patch", "append_slide table requires columns and rows.")
...
def _optional_nonnegative_float(value: Any, field_name: str) -> float | None:
    ...
    try:
        ...
        raise OKofficeException("invalid_patch", f"{field_name} must be a number.") from exc
    if parsed < 0:
        raise OKofficeException("invalid_patch", f"{field_name} must be non-negative.")
    return parsed
...
def _optional_nonnegative_int(value: Any, field_name: str) -> int | None:
    ...
    try:
        ...
        raise OKofficeException("invalid_patch", f"{field_name} must be an integer.") from exc
    if parsed < 0:
        raise OKofficeException("invalid_patch", f"{field_name} must be non-negative.")
    return parsed
