...
from okoffice.schemas.models import OKofficeError, Artifact, ToolResult
...
def run_workflow(workflow: Mapping[str, Any], dry_run: bool = False) -> ToolResult:
    ...
    executed_steps = 0
    failed_steps = 0
    artifact_dir = Path(str(workflow.get("artifact_dir") or f".okoffice-out/workflows/{run_id}"))
...
def _run_local_step(tool: str, payload: dict[str, Any]) -> ToolResult:
    from okoffice.tools import runner
    ...
    if tool == "office.workflow.plan":
        ...
        return runner.run_office_workflow_plan(
...
def _coerce_optional_int(value: Any, field_name: str, tool: str) -> tuple[int | None, ToolResult | None]:
    ...
    try:
        ...
        return None, ToolResult(
            ...
            status="failed",
            tool=tool,
            error=OKofficeError(code="unsafe_input_rejected", message=message),
            warnings=[message],
        )
