    ...
    if existing_source_map:
        for mapping in existing_source_map:
            if not isinstance(mapping, dict):
                continue
            block = block_index.get(str(mapping.get("block_id") or ""))
...
def _load_optional_payload(value: dict[str, Any] | str | Path | None, label: str) -> dict[str, Any] | None:
    ...
    path = Path(value)
    if not path.exists():
        raise AgentPDFException("file_not_found", f"{label} JSON not found: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise AgentPDFException("invalid_input", f"{label} JSON must be an object.")
    return payload
