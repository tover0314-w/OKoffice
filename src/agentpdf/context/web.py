...
from agentpdf.context.packet import _context_item_result, _web_citation_evidence
...
def capture_web_context(
    ...
    *,
    ...
    if timeout_seconds <= 0 or timeout_seconds > 60:
        raise AgentPDFException("unsafe_input_rejected", "timeout_seconds must be between 0 and 60.")
    ...
    request = Request(
        ...
        headers={"User-Agent": "AgentPDF-Local/0.1"},
    )
    with urlopen(request, timeout=timeout_seconds) as response:
        ...
        if final_parsed.scheme.lower() not in {"http", "https"}:
            raise AgentPDFException("unsafe_input_rejected", "Web capture final URL must use http:// or https://.")
        if final_parsed.username or final_parsed.password:
            raise AgentPDFException("unsafe_input_rejected", "Web capture final URL must not include credentials.")
...
def _validate_public_url_host(hostname: str | None, allow_private_hosts: bool) -> None:
    ...
    for address in addresses:
        ...
        if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_multicast:
            raise AgentPDFException("unsafe_input_rejected", "Private or local URL hosts are blocked by default.")
