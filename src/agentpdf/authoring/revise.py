...
from agentpdf.authoring.models import DesignTokens, PageDocument
...
def _apply_revisions(page_document: dict[str, Any], revisions: list[Mapping[str, object]]) -> list[int]:
    ...
    if not isinstance(pages, list):
        raise AgentPDFException("authoring_invalid_page_document", "Page document must include a pages array.")
    ...
    for raw_revision in revisions:
        if not isinstance(raw_revision, Mapping):
            raise AgentPDFException("unsafe_input_rejected", "Each page revision must be an object.")
...
def _page_number(raw_revision: Mapping[str, object]) -> int:
    ...
    if page_number < 1:
        raise AgentPDFException("invalid_page_range", "Revision page_number must be at least 1.")
    return page_number
