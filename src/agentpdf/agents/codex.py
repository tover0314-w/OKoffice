...
def setup_codex(
    ...
    """Build a Codex-friendly MCP config for the local AgentPDF server."""
    ...
    usage: dict[str, Any] = {
        ...
        "starter_prompt": (
            "Use the local agentpdf MCP server. Read AGENTS.md, call agentpdf_tool_manifest, "
...
def build_codex_mcp_config(
    ...
    if not command:
        raise AgentPDFException("unsafe_input_rejected", "Codex MCP command cannot be empty.")
    if not server_name:
        raise AgentPDFException("unsafe_input_rejected", "Codex MCP server name cannot be empty.")
    if not safe_root:
        raise AgentPDFException("unsafe_input_rejected", "Codex safe root cannot be empty.")
