...
def parse_page_range(spec: str, total_pages: int) -> list[int]:
    ...
    if total_pages < 1:
        raise AgentPDFException("invalid_page_range", "PDF has no pages to select.")
    if normalized == "all":
    ...
    if not normalized:
        raise AgentPDFException("invalid_page_range", "Page range cannot be empty.")

    pages: list[int] = []
    for token in normalized.split(","):
        part = token.strip()
        if not part:
            raise AgentPDFException("invalid_page_range", f"Invalid empty page range in {spec!r}.")
        if "-" in part:
...
def _parse_span(part: str, total_pages: int) -> list[int]:
    ...
    if separator != "-" or not raw_start or not raw_end:
        raise AgentPDFException("invalid_page_range", f"Invalid page span {part!r}.")
    start = _parse_page_number(raw_start, total_pages)
    end = _parse_page_number(raw_end, total_pages)
    if end < start:
        raise AgentPDFException("invalid_page_range", f"Page span {part!r} is reversed.")
    return list(range(start, end + 1))


def _parse_page_number(raw: str, total_pages: int) -> int:
    if not raw.isdigit():
        raise AgentPDFException("invalid_page_range", f"Invalid page number {raw!r}.")
    page = int(raw)
