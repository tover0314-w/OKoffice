...
def _normalize_operations(operations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not isinstance(operations, list) or not operations:
        raise AgentPDFException("invalid_input", "Deck patch operations must be a non-empty list.")
    normalized = []
    for index, operation in enumerate(operations, start=1):
        if not isinstance(operation, dict):
            raise AgentPDFException("invalid_input", f"Deck patch operation {index} must be an object.")
        op = str(operation.get("op") or "")
        if op == "replace_text":
            find = str(operation.get("find") or "")
            replace = str(operation.get("replace") or "")
            if not find:
                raise AgentPDFException("invalid_input", f"Deck patch operation {index} requires find text.")
            ...
            raise AgentPDFException("invalid_input", f"Unsupported Deck patch operation: {op or '<missing>'}")
    return normalized
