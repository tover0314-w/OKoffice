...
from agentpdf.office.word import WORD_DOCUMENT, W_NS, inspect_word_document
...
def plan_word_patch(*, input_path: str | Path, operations: list[dict[str, Any]]) -> ToolResult:
    try:
        ...
        return _failed(PLAN_TOOL, AgentPDFError(code="invalid_input", message=str(exc)))
...
def apply_word_patch(
    *,
    ...
    try:
        ...
        return _failed(APPLY_TOOL, AgentPDFError(code="invalid_input", message=str(exc)))
...
def _normalize_operations(operations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not isinstance(operations, list) or not operations:
        raise AgentPDFException("invalid_input", "Word patch operations must be a non-empty list.")
    normalized = []
    for index, operation in enumerate(operations, start=1):
        if not isinstance(operation, dict):
            raise AgentPDFException("invalid_input", f"Word patch operation {index} must be an object.")
        op = str(operation.get("op") or "")
        if op not in {"replace_paragraph", "append_paragraph", "insert_paragraph_after"}:
            raise AgentPDFException("invalid_input", f"Unsupported Word patch operation: {op or '<missing>'}")
        text = str(operation.get("text") or "")
        if not text:
            raise AgentPDFException("invalid_input", f"Word patch operation {index} requires non-empty text.")
...
def _apply_operations(
    ...
    body = document.find(f"{W_NS}body")
    if body is None:
        raise AgentPDFException("invalid_input", "Word document has no body.")
