...
def inspect_health_pdf(input_path: str | Path) -> ToolResult:
    ...
    usage: dict[str, Any] = {
        ...
        "scanner": "agentpdf_pdf_health_static_v1",
        "findings": findings,
...
def sign_pdf(
    ...
    output = resolve_output_path(output_path)
    payload = {
        "signature_format": "agentpdf_detached_sha256_v1",
        "input": str(source),
        "sha256": digest,
        "signature": signature,
...
def malware_scan_pdf(input_path: str | Path) -> ToolResult:
    ...
    return ToolResult(
        ...
        usage={
            "input": str(source),
            "scanner": "agentpdf_static_pdf_marker_scan_v1",
            "suspicious_count": len(findings),
            "findings": findings,
        },
...
def _encrypt_result(
    ...
    if not password:
        raise AgentPDFException("unsafe_input_rejected", "A non-empty password is required.")
    source = resolve_input_path(input_path)
    reader = _reader(source)
    writer = PdfWriter()
    for page in reader.pages:
...
def _decrypt_result(
    ...
    if not reader.is_encrypted:
        raise AgentPDFException("invalid_password", "Input PDF is not encrypted.")
    if reader.decrypt(password) == 0:
        raise AgentPDFException("invalid_password", "Password did not unlock the PDF.")
    writer = PdfWriter()
    for page in reader.pages:
        writer.add_page(page)
    if reader.metadata:
...
def _write_redacted_raster_pdf(
    ...
    if render_scale <= 0:
        raise AgentPDFException("unsafe_input_rejected", "render_scale must be greater than zero.")
...
def _finite_float(value: Any, label: str) -> float:
    try:
        ...
        raise AgentPDFException("unsafe_input_rejected", f"{label} must contain numbers.") from exc
    if not math.isfinite(number):
        raise AgentPDFException("unsafe_input_rejected", f"{label} must contain finite numbers.")
    return number
...
def _parse_fill_color(value: str) -> tuple[int, int, int]:
    ...
    try:
        ...
        raise AgentPDFException("unsafe_input_rejected", f"Invalid fill color: {value}") from exc
    if len(rgb) == 4:
        return rgb[:3]
    return rgb
